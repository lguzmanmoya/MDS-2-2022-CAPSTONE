...
class AgeEstimator():
    def __init__(self, weights=None, device='cpu'):
        ...
        self.model = Model().to(self.device)  # Asegúrate que 'Model' esté correctamente definido e importado.
        if weights:
    ...
    def predict_age(self, filename):
        transform = T.Compose([
            T.Resize((64, 64)),  # Asegúrate que esta es la dimensión correcta para tu modelo.
            T.ToTensor(),
        ...
        with torch.no_grad():
            _, age_estimates = self.model(image)
            predicted_age = age_estimates.argmax().item()  # Obtiene el índice de la edad más probable.
